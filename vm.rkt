...
(define-language vm
  ...
  (R hole (lift (lam x R)) (if (code e) R g) (if (code e) v R) (run (code e) R))
...
(define red
  (reduction-relation
   ...
   (--> (in-hole E (b_0 (code e_1) (code e_2)))
        (in-hole E (reflect (code (b_0 e_1 e_2))))                        "bc")
   (--> (in-hole E (app (code e_1) (code e_2)))
        (in-hole E (reflect (code (app e_1 e_2))))                        "appc")
   (--> (in-hole E (lift (lit number_1)))
        (in-hole E (code (lit number_1)))                                 "lift-lit")
   (--> (in-hole E (lift (cons (code e_1) (code e_2))))
        (in-hole E (reflect (code (cons e_1 e_2))))                       "lift-pair")
   ...
   (--> (in-hole R (reify e)) (in-hole R e)                               "reify-id"
        (side-condition (no-reflect? (term e))))
   ))
